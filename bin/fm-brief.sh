...
State the PR-facing intent and description as the feature's current, final accepted behavior for a reviewer, never as a chronology of validation rounds, prior fixes, or process history.
A reviewer can already see round-by-round changes in commit history, so narrate the accepted outcome once, not the journey to reach it.
